...
dic = load(joinpath(@__DIR__, "Data", "2-Logistic.jld2"))
...
for (j,d) in enumerate(d_list)
    ...
    push!(df, (d,"BPS (optimally tuned)", 1/bci[1][1], 1/bci[1][2], 1/bci[1][3]))
    bs = bootstrap(mean, SE_FECMC[:,j], BasicSampling(5000))
    bci = confint(bs, BCaConfInt(0.95))
    push!(df, (d,"FECMC",1/bci[1][1], 1/bci[1][2], 1/bci[1][3]))
end
