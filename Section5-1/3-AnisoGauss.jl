...
const OUTPUT_PATH = joinpath(@__DIR__, "Data", "3-AnisoGauss.jld2")
...
@inline function U(x::AbstractVector)
  s = sum(x)
  nrm2 = sum(abs2, x)
  return nrm2 / 2 / (1-γ) - γ / (1-γ) / (1-γ+γ*d) * s^2 / 2
end
...
function Experiment_once(d::Int, T::Float64, FECMC, BPS_sampler)
  # Σ = (1-γ)I + γ 11'  (compound symmetry)
  # Sample without materializing Σ or constructing MvNormal (saves allocations):
